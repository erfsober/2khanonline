...
@section('title', 'خرید محصولات ' . $brand->name . ' در شیراز | Smokify')
@section('seo_title', 'خرید محصولات ' . $brand->name . ' در شیراز | اسموکیفای')
@section('seo_description', 'مشاهده و خرید محصولات برند ' . $brand->name . ' در شیراز از فروشگاه Smokify | اسموکیفای با ارسال سریع.')
    ...
    <section class="pt-8 pb-20">
        <div class="max-w-[1260px] mx-auto px-6 lg:px-8">
            <nav class="flex items-center gap-2 text-xs text-[#737373] mb-6" aria-label="breadcrumb">
                ...
                <span class="text-[#171717] font-medium">محصولات {{ $brand->name }}</span>
            </nav>

            <div class="flex items-end justify-between gap-4 mb-8">
                <div>
                    <p class="text-sm text-[#737373] mb-2">برند</p>
                    <h1 class="text-2xl lg:text-3xl font-bold">محصولات {{ $brand->name }}</h1>
                </div>
                <span class="text-sm text-[#737373]">{{ number_format($products->total()) }} محصول</span>
            </div>

            <form method="GET" action="{{ route('brands.show', ['brand' => $brand->slug]) }}" class="bg-white border border-[#E5E5E5] rounded-2xl p-4 mb-8 flex flex-col md:flex-row md:items-end gap-4">
                <div class="flex-1">
                    <label for="category" class="block text-xs font-medium text-[#737373] mb-2">دسته‌بندی</label>
                    <select id="category" name="category" class="w-full rounded-lg border border-[#E5E5E5] bg-[#FAFAF9] px-3 py-2.5 text-sm focus:outline-none focus:border-[#171717]">
                ...
                @if ($categoryId > 0 || $sort !== '')
                    <a href="{{ route('brands.show', ['brand' => $brand->slug]) }}" class="text-center text-sm text-[#737373] hover:text-[#171717] transition-colors md:pb-2.5">
                        حذف فیلترها
                    </a>
                @endif
            </form>
